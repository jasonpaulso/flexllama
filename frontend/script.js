...
function createRunnerCard(runnerName, data) {
  ...
  card.innerHTML = `
        ...
        <div class="runner-info">
            ...
            ${
              isActive
                ? `<div><strong>URL:</strong> http://${host}:${port}</div>`
                : ""
            }
        </div>
  ...
  return `
        <div class="model-item status-${statusInfo.color}">
            ...
            <div class="model-info">
                <div class="model-name" title="${sanitizeHTML(
                  alias
                )}">${sanitizeHTML(alias)}</div>
                <div class="model-status status-${statusInfo.color}">${
    statusInfo.label
  }</div>
                <div class="model-message">${sanitizeHTML(
...
function createControlPanel(runnerName, data, runnerSlug) {
  ...
  return `
        <div class="runner-controls" data-runner="${sanitizeHTML(
          runnerName
        )}" data-runner-slug="${runnerSlug}">
            <div class="control-buttons">
                <button class="control-button btn-start ${
                  isOperating && operationType === "start" ? "loading" : ""
                }" 
                        data-action="start" 
                        ${!canStart ? "disabled" : ""}>
                    <span class="btn-icon">${
                ...
                <button class="control-button btn-stop ${
                  isOperating && operationType === "stop" ? "loading" : ""
                }" 
                        data-action="stop" 
                        ${!canStop ? "disabled" : ""}>
                    <span class="btn-icon">${
                      isOperating && operationType === "stop"
                        ? loadingIcon
                        : stopIcon
                    }</span>
                    <span class="btn-text">${
                ...
                <button class="control-button btn-restart ${
                  isOperating && operationType === "restart" ? "loading" : ""
                }" 
                        data-action="restart" 
                        ${!canRestart ? "disabled" : ""}>
                    <span class="btn-icon">${
                      isOperating && operationType === "restart"
                        ? loadingIcon
                        : restartIcon
                    }</span>
                    <span class="btn-text">${
            ...
            <div class="control-status ${
              operationState ? operationState.statusClass : ""
            }" id="status-${runnerSlug}">${
    operationState ? operationState.message : ""
  }</div>
        </div>
    `;
}
